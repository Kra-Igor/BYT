...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			//accountlist.get(accountid); //KTR del -> testOpenAccount powinien rzuciæ wyj¹tkiem, nie rzuci³
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
			...
			if (!accountlist.containsKey(accountid)) { //KTR add (brakuj¹ca negacja)
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			//account.deposit(money); //KTR del -> przy przegl¹daniu metody do testu jednostkowego
	...
	public void addTimedPayment(String accountid, String payid, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid) || !tobank.accountlist.containsKey(toaccount)) throw new AccountDoesNotExistException(); //KTR add -> testTimedPayment -> powinien rzucaæ wyj¹tkiem AccountDoesNotExistException
		Account account = accountlist.get(accountid);
	...
	public void removeTimedPayment(String accountid, String id) throws AccountDoesNotExistException{
		if(!accountlist.containsKey(accountid)) throw new AccountDoesNotExistException(); //KTR add -> testTimedPayment -> powinien rzucaæ wyj¹tkiem AccountDoesNotExistException
